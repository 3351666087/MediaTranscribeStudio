...
11. 本地 LLM 与声学模型职责分离，但必须进入联合决策闭环。当前 M4 的下一候选是 `qwen3.5:9b` Q4_K_M；服务器质量档至少评测 `Qwen3.5-35B-A3B`，资源允许时再评测 `122B-A10B`。第一条语义路径在已有 acoustic top-K speaker states、原生时间线和 ASR token 时间内重排 who-said-what，利用问答关系、未完句连续性、称谓、角色和话题连贯性修复 orchestration 错位；第二条路径在 ASR N-best、强制对齐、术语表和邻接上下文内生成最小语法/同音词修复。LLM 不能创建新说话人、移动声学边界、覆盖重叠轨道、修改 `rawText` 或生成原音频无证据的词，低置信冲突必须 abstain 并进入人工复核。
...
| CPU 边缘联合 ASR | MOSS-Transcribe-Diarize 0.9B CPU/FP32 高优先级挑战；原始精度服务器旗舰保留质量锚点 | VibeVoice-ASR-BitNet、whisper.cpp 量化候选 | MOSS 在 AISHELL-4 N=5 改善 who/what/边界但 overlap 明显回退；1.5B BitNet 是 7B 派生效率模型且公开报告有绝对 WER 损失；边缘胜出不等于基模胜出 |
...
| 语义说话人纠错 | 声学 top-K/候选时间线 + 本地 LLM 约束重排 + transcript-preserving transfer | Qwen3.5-9B/35B；DiarizationLM 仅作英语研究挑战 | 零样本 LLM 可显著恶化 diarization；单 ASR/单数据集微调存在跨 ASR 与跨域回退，不能让语义自由改人 |
| 语法与同音词修复 | 不可变 `rawText` + ASR N-best/强制对齐/术语证据 + 本地 LLM 最小 patch | 9B 边缘建议、35B/122B 服务器挑战 | “读起来更顺”不证明声学正确；无候选证据的内容词修改只能人工确认 |
...
flowchart TD
  ...
  SPECIALIST --> ALIGN
  OMNI --> ALIGN
  ALIGN --> NBEST["不可变声学 N-best、token 时间与校准置信度"]
  SEG --> JOINT["MOSS 0.9B / VibeVoice 7B 全局联合挑战"]
  JOINT --> NBEST
  FUSION --> SEMANTIC["本地 LLM 语义说话人候选重排"]
  NBEST --> SEMANTIC
...
- `qwen3.5:9b` 已安装为 Ollama model ID `6488c96fa5fa`，实际为 9.7B、Q4_K_M、6.6 GB；底层 blob SHA-256 为 `dec52a44569a2a25341c4e4d3fee25846eed4f6f0b936278e3a3c900bb99d37c`。只有声学阶段结束后才加载。4B 已因正式质量门禁失败退出生产 allowlist 并从本机卸载；9B 不可用时必须 fail closed，不得静默降级到已拒绝模型。数据卷始终保留至少 8 GiB 工作空间。
...
- MOSS 的本地挑战现统一由有界隔离 runner 执行：固定模型 revision 与 19 文件全量 SHA-256，固定 Python/Torch/Transformers 等运行时版本，强制离线，独立进程组每 2 秒发送 stage/token heartbeat，同时采集进程树 RSS、macOS physical footprint 和系统内存压力；idle timeout、不可刷新的 hard deadline、资源上限、输出长度和媒体时间边界任一触发即终止完整进程组。MPS/FP16 在 AISHELL-4 `N=5` 生成 286 tokens 时 footprint 达 `16 GiB`、系统可用内存降至 `4%`，已经按 `14 GiB` 上限 fail closed；当前 M4 16 GB 禁用 MOSS MPS 路由，不能通过放宽门禁继续尝试。
- 同一 AISHELL-4 `N=5 / 19.54 秒 / 11 turns / 4.5593 秒 overlap` 在 CPU/FP32 上于 `31.769 秒`墙钟、`27.773 秒`推理内完成 299 tokens/13 段，人数 `5/5`、DER/JER `0.194433903 / 0.227918105`、speaker confusion `0.044022770`、cpCER/tcpCER/SA-CER 均为 `0.262135922`，边界 mean/p50/p95/max `98.95/60.5/316.45/508 ms`，峰值 RSS/footprint `4445.42/4418 MB` 且没有时间越界。相较当前 guarded production v11b，who/what/边界显著改善，但 overlap F1 从 `0.686414033` 退化到 `0.450516330`，Transformers 输出也没有独立语言字段。因此 MOSS CPU/FP32 只晋级为按不确定性触发的高优先级联合候选：其 who/what/timing 可进入校准仲裁，Community-1 regular timeline 继续负责 overlap 权威，二者都不得被 LLM 自由改写。
...
- 模型生命周期维护除 Ollama 外还逐项登记本地模型、量化版本、adapter 与隔离 runtime。大型评测和每次模型晋级/拒绝后都先做引用审计，再只对清单中 `retired` 且不被生产、明确回退、冻结基准或不可再生证据引用的精确目录执行删除；可再生失败输出和下载缓存优先清理，清理后必须复核 `actionRequired=false` 和至少 `8 GiB` 工作空间。MOSS 活动权重/运行时继续保留，被替代的 File Provider venv 已实际卸载。
...
| 联合说话人转录 | cpWER、tcpWER、SA-WER、WDER、漏说话人率；不能只分别报 DER 和 WER |
...
| LLM 语义纠错 | candidate-state 命中率、speaker proposal precision/recall、SA-WER/tcpWER delta、edit-span P/R/F1、受保护 token 保留率、semantic drift、abstention |
...
3. MOSS-Transcribe-Diarize 0.9B 的快照/remote-code 审计、有界隔离 runner 和本机真实 N=5 CPU/FP32 联合挑战已经完成；M4 禁用其 MPS 路由，CPU 只保留按分桶触发的候选。下一步补齐真实 N=3、代码切换、长音频和更多 overlap 分桶，并在旗舰 GPU 服务器用官方 SGLang/vLLM 与 Community-1 + Qwen 级联及 VibeVoice-ASR-7B 做同音频挑战。必须同时比较 cpWER/tcpWER、DER/JER、漏轨、overlap、时间边界、幻觉、逐段语言和资源，不因参数规模小而降低其优先级，也不以本机 CPU/MPS 结果代替官方 GPU 基模上限。
...
- [DiarizationLM](https://arxiv.org/abs/2401.03506) 与 [Google 开源 8B Fisher v2 model card](https://huggingface.co/google/DiarizationLM-8b-Fisher-v2) 证明微调 LLM 可作为 transcript-preserving speaker post-processor，在 Fisher/Callhome 英语上降低 WDER；checkpoint revision `668ce4eda93af6333c69aaf65d5bb1f3ac1d1433`、Llama 3 许可，6000 字符/4096 token 上下文，不能代表多语、多人或任意 ASR。
- [LLM-based speaker diarization correction: A generalizable approach](https://arxiv.org/abs/2406.04927) 报告零样本 7B/8x7B 明显恶化 diarization，ASR 专用微调在未见 ASR 上也可能回退；它支持“语义纠错必须按上游 ASR 和场景 held-out、默认 suggestion-only”的门禁，而不是否定 LLM 协同价值。
