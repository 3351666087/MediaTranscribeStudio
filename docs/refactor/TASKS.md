...
- [x] 2026-07-27 更新本机模型与可再生缓存磁盘审计：数据卷在本轮运行期间最低约 `12 GiB` 可用，删除无进程引用且可重建的旧 Python 验证环境与失败 v9 临时运行约 `1.7 GiB` 后先回升到约 `13 GiB`，APFS 释放可回收空间后的最终审计约 `19 GiB`。现行 7 个生产模型约 `6.8 GiB`，全部被 guarded 生产配置引用；Ollama `qwen3.5:9b` 约 `6.6 GB` 为默认高能力候选，`qwen3.5:4b` 约 `3.4 GB` 虽未晋级但仍是架构登记的有界回退，因此没有把仍被引用的模型伪装成“淘汰模型”删除。补齐锁定的 `meeteval==0.4.3` 只增加约 `2.7 MiB`，v11b 不可再生证据只持久化约 `1.1 MiB`。后续每次模型晋级/拒绝和大型评测结束都必须重跑引用审计；只有不再被生产配置、显式回退、冻结基准或不可再生证据引用的权重才立即卸载，可再生成的失败输出和下载缓存优先清理，始终保留至少 `8 GiB` 工作空间。
