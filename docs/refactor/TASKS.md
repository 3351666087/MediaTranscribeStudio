...
- [x] 将 5 条 FLEURS 精确切换样本的每案例上限收紧到 `40 秒`，离线重建后的实际时长为 `33.30–39.36 秒`，同时保留全局 `90 秒` 上限供确实需要覆盖 `N=5` 真值的自然会话使用。构建器新增版本化 `source-evidence` 缓存，严格绑定 dataset、revision、许可证和来源字段，并将证据路径与 SHA-256 写入 resolved manifest；缓存不匹配或损坏时 fail closed。首次联网构建与随后把 `HTTP(S)/ALL_PROXY` 指向不可达端口的离线重建均为 `19/19`、`failedCases: []`，聚焦回归 `10 passed`，证据位于 `.runtime_cache/sample-library/code-switch-short-v2/`。
