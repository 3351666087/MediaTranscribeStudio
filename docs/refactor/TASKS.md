...
- [x] 按模型生命周期规则淘汰本机 `qwen3.5:4b`：其正式 88 样本结论已是 `reject_for_production`，因此从 Python 生产配置、worker 请求契约、桌面请求契约和策略元数据中移除；9B 不可用时统一 fail closed，不允许降级到已拒绝模型。历史 benchmark/测试 fixture 只保留质量拒绝证据，不构成运行时引用；本机 Ollama 权重已卸载。桌面策略改为准确显示 9B 的强制 fail-closed `suggestion_only` 状态。
