...
- [x] 修复 AISHELL-4 密集连续字幕首次 fail-closed 暴露的累计时间漂移：`tools/export_sample_subtitles.py` 先按默认 `80 ms / 17 字符每秒` 策略排版；仅当源段已单调不重叠且默认结果越过持久化媒体时长时，才使用有审计记录的源时间轴回退（`gap=0`，不伪造媒体时长），并把默认策略 QA 结果写入 acceptance。真实 19.54 秒样本生成带说话人标签的 21 个 SRT/WebVTT/ASS cue，首尾 `0–19520 ms`，严格 UTF-8、文本覆盖、单调不重叠和 `<=19540 ms` 均通过，墙钟 `0.70 秒`；acceptance SHA-256 为 `f1432e5dc0e9ec821cab67cd71547bfd0480c7e4d061048df3ec4c4275d0cd9b`，位于 `.runtime_cache/outputs/global-sample-library/aishell4-n5-unapproved-subtitle-preview-v1/`。默认可读性策略真实失败并完整保留：20 个零间隙告警、7 个阅读速度超限，最大 `19.543973941` 字符每秒；正反向聚焦回归 `17 passed`，受控 `TMPDIR` 全仓门禁 `1063 passed, 20 skipped, 162 subtests passed`。这只算技术预览成功，视觉/语言/说话人/发布质量均未通过，soft-mux、burn-in 和代表帧像素检查仍待真实视频闭环。
