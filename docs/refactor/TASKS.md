...
- [ ] guarded overlap recovery 的 AISHELL-4 真实生产晋级仍未通过。离线 ablation 只找到 `ovl-01-ch2 -> speaker-5` 一条 margin `0.301915262` 的合格 secondary channel，追加后 cpCER/tcpCER/SA-CER 从 `0.398058252 / 0.436893204 / 0.398058252` 改善到 `0.359223301 / 0.398058252 / 0.359223301`，DER/JER 从 `0.390554501 / 0.504293093` 改善到 `0.360826481 / 0.486928561`，边界 mean 从 `923.045455 -> 765.454545 ms`；这不是端到端晋级证据。真实 v1 的原始主 ASR 能完成，但未限长的分离候选 ASR 超过 `1800 秒`；v2-v7 对正常主 ASR 的 batching/token 改动反而均在 overlap recovery 前超时，已撤回。v8 保留 provider 默认主批处理并只限制分离候选，仍在主 transcription heartbeat 到 `1787.213 秒` 后触发 `JOB_HARD_DEADLINE_EXCEEDED`，总墙钟 `1803.273 秒`，未进入 CAM++/MossFormer2 且强制清理 worker `72238/72366`。因此当前 PyTorch MPS Qwen3-ASR 路径不满足该分桶可落地时延，停止继续做 token 参数试错，下一步必须在同一不可变音频上比较 CPU 与 MLX/Whisper 挑战者并同时保留质量指标。
- [x] 2026-07-27 完成本机模型与可再生缓存磁盘审计：数据卷约 `20 GiB` 可用；现行 7 个生产模型约 `6.9 GiB`，全部被 guarded 生产配置引用，未发现可卸载的生产权重。Ollama `qwen3.5:9b` 为当前默认高能力候选；`qwen3.5:4b` 虽已拒绝生产晋级，但架构文档仍明确把它保留为 9B 完整 held-out 前的有界回退，因此本轮不伪装成“淘汰模型”删除。v2-v7 可再生失败运行已移到精确隔离的废纸篓目录，仅保留 v1 根因和 v8 终止证据。后续每次模型晋级/拒绝和大型评测结束都必须重跑引用审计；只有不再被生产配置、显式回退、冻结基准或不可再生证据引用的权重才立即卸载，可再生成的失败输出和下载缓存优先清理，始终保留至少 `8 GiB` 工作空间。
